...
export default function Dashboard({ user, userDoc }) {
  ...
  const handleActualizar = async () => {
    ...
    try {
      ...
      if (!secretSnap.exists()) throw new Error('Falta configurar el token.');
      ...
      if (res.status === 204) {
        setRefreshMessage({ type: 'success', text: 'Corrida disparada. Tarda 1-2 minutos.' });
      } else {
  ...
  return (
    <div className="px-6 py-6 space-y-4">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-3 flex items-center justify-between flex-wrap gap-3">
        <BcvAndRun bcv={bcv} ultimaCorrida={ultimaCorrida} isAdmin={isAdmin} />
...
function BcvAndRun({ bcv, ultimaCorrida, isAdmin }) {
  ...
  const handleSave = async () => {
    ...
    if (ok) { setBcvInput(''); setEditingBcv(false); }
  };
      ...
      <div className="flex items-center gap-2">
        ...
        {editingBcv ? (
          <>
            ...
            <button onClick={handleSave} className="text-xs px-2 py-1 bg-blue-600 text-white rounded">Guardar</button>
